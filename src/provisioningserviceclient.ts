...
export class ProvisioningServiceClient {
   ...
   */
    public getIndividualEnrollmentAttestationMechanism(enrollmentId: string, callback: HttpResponseCallback<AttestationMechanism>): void;
    public getIndividualEnrollmentAttestationMechanism(enrollmentId: string): Promise<ResultWithHttpResponse<AttestationMechanism>>;
    public getIndividualEnrollmentAttestationMechanism(enrollmentId: string, callback?: HttpResponseCallback<AttestationMechanism>): Promise<ResultWithHttpResponse<AttestationMechanism>> | void  {
    ...
    if (!enrollmentId) {
      throw new ReferenceError('enrollmentId cannot be \'' + enrollmentId + '\'');
    }
    ...
    Authorization: <sharedAccessSignature>
    ```]*/
    const path = '/enrollments/' + encodeUriComponentStrict(enrollmentId) + '/attestationmechanism' + this._versionQueryString();
    const headers = {};
   ...
   *
   * @param enrollmentGroupId Unique identifier of the EnrollmentGroup.
   * @param callback Function called when the request is completed, either with an error or with an AttestationMechanism object.
   * @returns {Promise<ResultWithHttpResponse<AttestationMechanism>> | void} Promise if no callback function was passed, void otherwise.
   */
  ...
  public getEnrollmentGroupAttestationMechanism(enrollmentGroupId: string, callback?: HttpResponseCallback<AttestationMechanism>): Promise<ResultWithHttpResponse<AttestationMechanism>> | void  {
    /*Codes_SRS_NODE_PROVISIONING_SERVICE_CLIENT_16_003: [The `getEnrollmentGroupAttestationMechanism` method shall throw a `ReferenceError` if the `enrollmentGroupId` parameter is falsy.]*/
    if (!enrollmentGroupId) {
  ...
  static fromConnectionString(value: string): ProvisioningServiceClient {
    ...
    const config: RestApiClient.TransportConfig = {
      host: cn.HostName,
      sharedAccessSignature: SharedAccessSignature.create(cn.HostName, cn.SharedAccessKeyName, cn.SharedAccessKey, Date.now()) as any,
